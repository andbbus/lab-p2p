# Peer-to-Peer Lab (Spring 2026)

## Overview

Welcome to the **Peer-to-Peer Lab**

This initiative is designed as a hands-on, collaborative series of workshops geared towards students in Economics & Development who want practical skills for conducting impactful research.

**Spring 2026 Features:**
- 5 focused sessions
- Targeted at students working on thesis/research projects
- Peer-to-peer learning approach
- Immediate application to your own work
- Small cohort size (12-15 participants)

---

## Objectives

By the end of the program, participants will have:
- **Research proposal** using structured methodology
- **LaTeX template** for academic writing
- **GitHub portfolio** with version-controlled projects
- **Literature matrix** created with LLM assistance
- **Publication-ready visualizations** for research
- **Interactive dashboard** showcasing findings

---

## Program Structure

**6 Sessions (90 minutes each):**

### Session 1: Research Project Development
**Focus:** How to develop a research project and address research questions
- Research question formulation (SMART criteria)
- Literature gap identification
- Methodology selection frameworks
- **Output:** One-page research proposal template

### Session 2: LaTeX & Academic Writing (Overleaf)
**Focus:** Professional document preparation
- Document structure and formatting
- Citations and bibliography management
- Mathematical notation and figures
- **Output:** Conference paper template

### Session 3: Version Control with GitHub
**Focus:** Collaborative research workflows
- Repository setup and management
- Branching and merging strategies
- Documentation best practices
- **Output:** Personal research portfolio repo

### Session 4: LLM-Assisted Literature Review
**Focus:** AI tools for research efficiency
- Systematic search strategies
- AI-powered synthesis techniques
- Ethical considerations and citation
- **Output:** Structured literature matrix

### Session 5: Data Visualization with ggplot2
**Focus:** Static publication-ready charts
- Grammar of graphics principles
- Economic data visualization patterns
- Export and formatting for papers
- **Output:** Publication-ready figure set

---

## Session Format

Each session includes (roughly):
- **20 minutes:** Concept introduction and theory
- **60 minutes:** Hands-on practical activity
- **10 minutes:** Peer sharing and next steps
- **Take-home assignment** building toward final portfolio

---

## Who Should Join?

- Students preparing for their thesis or research project
- Anyone interested in reproducible, impactful data science workflows
- Perfect for Economics & Development students (but open to all)

---

## Prerequisites

- Basic familiarity with R – self study materials will be provided
- Laptop for hands-on exercises – eventually we could think of booking lab room

---

## Satisfactory metrics

**Primary Goals:**
- 12-15 enrolled participants (manageable cohort size)
- 80%+ retention rate through all 5 sessions
- 6 complete portfolios submitted at showcase

**Learning Outcomes:**
- All participants complete final showcase presentation
- Active community engagement via Discord
- Skills immediately applicable to thesis work

---

## Getting Started

### Registration
[Registration link will be available in February 2026]

### Setup Requirements
1. Install R and RStudio
2. Create GitHub account
3. Create Overleaf account
4. Join the Google Classroom (to setup)

---

## Project Timeline

- **November-December 2025:** Curriculum development and setup
- **January-February 2026:** Marketing and registration
- **March-May 2026:** 6 bi-weekly sessions
- **June 2026:** Program evaluation and iteration planning

---

## Future Vision

### If Spring 2026
- **Fall 2026:** Expanded 9-session program
- **Long-term:** Semester-long certificate program (let's see if bureaucracy is on our side..)
- **Scaling:** Integration with thesis supervision (or attendance somehow incouraged)
- **Network:** Alumni mentorship system

---

## Resources

### Key Learning Materials
- [R for Data Science](https://r4ds.had.co.nz/) - Data visualization chapters
- [Overleaf LaTeX Tutorial](https://www.overleaf.com/learn/latex/Learn_LaTeX_in_30_minutes)
- [GitHub Learning Lab](https://lab.github.com/)
- [ggplot2 Cheat Sheet](https://posit.co/wp-content/uploads/2022/10/data-visualization-1.pdf)

### Community
- **GitHub Issues:** Resource suggestions and session ideas
- **Email:** **TO DO**

---

## How to Contribute

Want to help improve this lab?
- Create an issue with suggested resources or exercises
- Volunteer as a session facilitator
- Share your expertise in specific topics
- Provide feedback on curriculum design

---

## License

This project is open-source and freely available for educational purposes.

---

## Contact

**Email:** andrea.busolo@edu.unifi.it
**GitHub Issues:** For suggestions and improvements  


## Archive: Full Program Documentation

For reference, the complete 12-session curriculum with extensive resources is preserved in the `archive/` folder. The MVP focuses on the 5 most essential skills for immediate research application.
